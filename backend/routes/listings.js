...
router.post("/", auth, async (req, res) => {
  try {
    const { title, description, startPrice, startTime, endTime, image, category, condition, targetGender } = req.body;
...
router.get("/popular", async (req, res) => {
  try {
    const popularListings = await Listing.find({ status: 'active' })
      .sort({ bidCount: -1 })
      .limit(5)
      .populate("seller", "name");
...
router.put("/:id", auth, async (req, res) => {
  try {
    const { title, description, startPrice, startTime, endTime, category, condition, targetGender } = req.body;
    const listing = await Listing.findById(req.params.id);
    ...
    const update = { title, description, startPrice, startTime, endTime, targetGender };
